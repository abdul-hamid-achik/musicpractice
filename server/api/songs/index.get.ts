...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (query.search && typeof query.search === 'string') {
      ...
      conditions.push(or(like(songs.title, searchTerm), like(songs.artist, searchTerm)));
    }
    ...
    if (query.instrumentType) {
      ...
      conditions.push(
        eq(songs.instrumentType, query.instrumentType as (typeof INSTRUMENT_TYPES)[number]),
      );
    }
