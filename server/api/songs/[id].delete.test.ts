/**
 * Tests for server/api/songs/[id].delete.ts
 */
...
import songDelete from './[id].delete';
...
describe('DELETE /api/songs/:id', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    ...
    const res = await songDelete(makeEvent());
    expect(res.statusCode).toBe(401);
  });

  it('returns 400 for missing id', async () => {
    useDbMock.mockReturnValue(makeDrizzleMock());
    const res = await songDelete(makeEvent(null));
    expect(res.statusCode).toBe(400);
  });

  it('returns 400 for invalid UUID format', async () => {
    useDbMock.mockReturnValue(makeDrizzleMock());
    const res = await songDelete(makeEvent('not-a-uuid'));
    expect(res.statusCode).toBe(400);
  });
