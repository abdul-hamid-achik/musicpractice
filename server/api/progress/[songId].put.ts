...
export default defineEventHandler(async (event) => {
  try {
    ...
    const songId = validateId(getRouterParam(event, 'songId'), 'songId');

    // Validate optional fields
    if (
      body.completionPercent != null &&
    ...
    if (
      body.practiceCount != null &&
      (!Number.isInteger(body.practiceCount) || body.practiceCount < 0)
    ) {
