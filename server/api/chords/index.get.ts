...
export default defineEventHandler(async (event) => {
  try {
    ...
    const where = query.instrumentType
      ? (() => {
          if (
            !INSTRUMENT_TYPES.includes(query.instrumentType as (typeof INSTRUMENT_TYPES)[number])
          ) {
            throw createApiError(
              `instrumentType must be one of: ${INSTRUMENT_TYPES.join(', ')}`,
              400,
            );
          }
          return eq(
            chords.instrumentType,
            query.instrumentType as (typeof INSTRUMENT_TYPES)[number],
          );
        })()
      : undefined;
