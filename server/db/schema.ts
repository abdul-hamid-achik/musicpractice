...
export const notationFormatEnum = pgEnum('notation_format', [
  'alphatex',
  'musicxml',
  'guitar_pro',
  'vexflow_json',
...
export const practiceGoals = pgTable(
  'practice_goals',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id')
...
export const earTrainingScores = pgTable(
  'ear_training_scores',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id')
...
export const metronomePresets = pgTable(
  ...
  (table) => [index('metronome_presets_user_id_idx').on(table.userId)],
);
