...
export function usePracticeSession() {
  ...
  const elapsed = ref(0);
  const currentSession = ref<{ instrumentId: string; songId?: string; tempoBpm?: number } | null>(
    null,
  );
    ...
    try {
      ...
      if (error instanceof DOMException) {
        if (error.name === 'QuotaExceededError') {
          storageError.value =
            'Storage quota exceeded. Please save your session or clear old data.';
          ...
          console.warn(
            '[usePracticeSession] localStorage unavailable (private browsing or disabled)',
          );
        } else {
  ...
  const {
    debouncedFn: debouncedPersist,
    cancel: cancelDebouncedPersist,
    flush: flushDebouncedPersist,
