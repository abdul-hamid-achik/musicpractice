...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function fetchUser() {
    try {
      // useRequestFetch forwards request headers (including cookies) during
      // SSR so the server can validate the same auth cookie the browser
      // holds. On the client it falls back to a regular $fetch.
