...
const totalWeeklyTarget = computed(() => {
  if (goals.value.length === 0) return 300;
  return goals.value.reduce((sum, g) => sum + g.targetMinutesPerWeek, 0);
});
...
<template>
  <div class="flex flex-col gap-4">
      ...
      <div class="space-y-4">
        ...
        <div>
          <div class="flex justify-between text-xs text-text-muted mb-1">
            ...
            <span
              >{{ Math.round((weeklyProgress.totalMinutes / totalWeeklyTarget) * 100) }}% of
    ...
    <!-- Empty state -->
    <div v-else-if="goals.length === 0" class="text-center py-8 text-text-muted">
    ...
    <StaggeredList v-else class="flex flex-col gap-4">
      <div v-for="goal in goals" :key="goal.id" class="bg-card border border-border rounded-lg p-4">
      ...
      <div class="flex gap-3">
        ...
        <div class="flex-1">
          ...
          >
            ...
            <option v-for="inst in instrumentStore.instruments" :key="inst.id" :value="inst.id">
              {{ inst.name }}
            </option>
          </select>
        </div>
