...
<template>
  <header
    class="fixed top-0 w-full bg-surface-alt border-b border-border z-40 h-16 flex items-center justify-between px-6"
    role="banner"
  >
    <div class="flex items-center gap-2">
      ...
      <NuxtLink
        to="/"
        class="flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-surface-alt rounded-md"
      >
    ...
    <div class="flex items-center gap-6">
      ...
      <div v-if="auth.isAuthenticated.value" class="flex items-center gap-3">
        <div
          class="w-8 h-8 rounded-full bg-primary/20 text-primary flex items-center justify-center text-sm font-bold"
          aria-hidden="true"
        >
          {{ auth.userName.value?.charAt(0)?.toUpperCase() }}
        </div>
        <span class="hidden sm:inline text-sm text-text font-medium">{{
          auth.userName.value
        }}</span>
        <button
