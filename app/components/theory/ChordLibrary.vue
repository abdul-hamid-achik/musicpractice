...
const chordNotes = computed(() => {
  ...
  return getChordNotes(
    selectedRoot.value,
    selectedChord.value.intervals.map((i) => i % 12),
  );
});
...
async function playChord() {
  ...
  try {
    ...
    // poly synth on every play (which leaks native audio nodes).
    ...
    const notes = chordNotes.value.map((n) => {
      const allNotes: string[] = getNoteNames();
  ...
  <div class="bg-card border border-border rounded-lg p-6 space-y-6">
    ...
    <div v-if="!isLoaded" class="space-y-4" aria-busy="true" aria-label="Loading chords...">
      ...
      <div class="flex gap-2">
        <UiSkeleton height="2rem" width="80px" />
        <UiSkeleton height="2rem" width="80px" />
      </div>
    </div>
      ...
      <div v-if="selectedChord" class="space-y-4">
        <div class="bg-surface rounded-lg p-4 border border-border">
          ...
          <div class="mb-3">
            ...
            <span class="text-text font-mono text-lg tracking-wider">{{
              chordNotes.join(' ')
            }}</span>
          </div>
