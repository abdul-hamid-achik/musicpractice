...
function handleNoteClick(payload: {
  note: string;
  string: number;
  finger: number;
  octave: number;
...
<template>
  <div>
    ...
    <p class="text-text-muted mb-6">
      Fingerboard with position guides — visualize scales and finger placements.
    </p>
    ...
    <div
      ...
      class="space-y-4"
      aria-busy="true"
      aria-label="Loading violin fingerboard..."
    >
        ...
        <div class="flex flex-wrap items-end gap-4">
          ...
          <div>
            ...
            <select
              ...
              class="bg-surface-alt text-text border border-border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
            >
              <option v-for="p in 7" :key="p" :value="p">
                {{ p }}{{ p === 1 ? 'st' : p === 2 ? 'nd' : p === 3 ? 'rd' : 'th' }} Position
              </option>
            </select>
          </div>
