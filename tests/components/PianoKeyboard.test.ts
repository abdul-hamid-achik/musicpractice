...
// Mock the useInstrumentSound composable so key clicks don't reach real Tone in happy-dom
...
describe('PianoKeyboard', () => {
  ...
  it('highlights notes when highlightedNotes prop is set', () => {
    ...
    const keysWithFill = whiteKeys.filter((key) => {
      ...
      return (
        fill && (fill.includes('var(--color-primary)') || fill.includes('var(--color-secondary)'))
      );
    });
